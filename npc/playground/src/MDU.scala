...
class MDU (implicit val conf: Configuration) extends Module {
  ...
  val result_aux = MuxCase(
    0.U, Array(
      (io.alu_op === ALU_DIV && io.alu_msk_type =/= ALU_MSK_W)    -> (io.alu_op1.asSInt / io.alu_op2.asSInt),
      (io.alu_op === ALU_DIV && io.alu_msk_type === ALU_MSK_W)    -> (io.alu_op1(31, 0).asSInt / io.alu_op2(31, 0).asSInt),
    )
  )

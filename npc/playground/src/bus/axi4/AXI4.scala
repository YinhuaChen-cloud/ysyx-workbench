...
//     RLAST ：标记最后一次读数据传输
//     可用于检验COUNT是否一致
//     RRESP ：读数据的反馈，用于传输总线错误。
//     简单核的实现可以在总线错误时不产生异常，只需要确保软件访问物理内存地址一定正确即
//     可。
//     RVALID ：读数据有效
//     RID 用于与 ARID 对应，用于标识对应的请求。
//   Master->Slave信号：
//     RREADY ：本次读数据传输完成
...
//     A[RW]LEN ：读写的传输次数，次数定义为 A[RW]LEN+1 。

//     A[RW]SIZE ：单次传输的数据大小，字节数定义为 2 ** A[RW]SIZE ，且不可超过数据总线
//     宽度。

//     A[RW]BURST ：突发传输类型（INCR与WARP模式可以一次地址握手完成多次不同地址的数
//     据传输，提高Cache替换效率，若只需要实现AXI4Lite可直接使用FIXED模式）

//     A[RW]VALID ：读写的有效性

//     此外，这里依据Slave支持的特性不同可能还有 A[RW]CACHE 、 A[RW]PROT 、 A[RW]PROT 、
//     A[RW]QOS 信号，这些信号我们可暂时忽略，直接assign为0即可

//     如果总线支持 A[RW]ID ，则需要对于不同的设备传输正确的ID信息。例如你的CPU的I-Cache与D-Cache独立，且都使用AXI访问总线，你就可以在CPU核里面写一个AXI Crossbar，并使用不同的

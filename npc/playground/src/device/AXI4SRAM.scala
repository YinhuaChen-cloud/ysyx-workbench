...
class AXI4SRAMnew extends CyhCoreModule {
  val io = IO(new Bundle {
    val pc_valid = Input(Bool())
    val pc = Input(UInt(XLEN.W))
    val pc_ready = Output(Bool())
  ...
  io.inst_valid := inst_valid_reg
  when(io.inst_ready === true.B) {
    inst_valid_reg  := false.B  // 一个周期之后变成false
  } .otherwise {
    inst_valid_reg  := true.B
  }
  // inst reading module
  val inst_read_module = Module(new INST_READ)
  ...
  io.inst := inst_reg
  inst_reg := Mux(io.inst_ready & io.inst_valid, inst_read_module.io.inst, inst_reg)
  // pc ready
  val pc_ready_reg = RegInit(false.B)
  io.pc_ready := pc_ready_reg
  when(io.pc_valid === true.B) {
    pc_ready_reg := true.B
  } .otherwise {
    pc_ready_reg := false.B
  }
  // for IFU-AXI4SRAM bus --- end

}

// 这个 inst_read 是瞬间就有(当下周期立刻完成，组合电路)
class INST_READ extends BlackBox with HasBlackBoxInline with HasCyhCoreParameter {
  val io = IO(new Bundle {
    val clk = Input(Clock())
    val rst = Input(Bool())
    val pc  = Input(UInt(XLEN.W))

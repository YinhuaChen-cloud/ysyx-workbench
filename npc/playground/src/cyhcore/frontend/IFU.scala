...
// （当然如果基础够好的同学可以直接实现AXI4的读写过程）。首先，我们的取指级（IF）应该发出取指信
// 号，包括读请求（valid）和读地址（pc），这时AXI模块应该收到取指级的信号，如果AXI模块处于空闲状态
// 则对本次读请求做出响应，AXI内部状态由空闲跳转到读请求状态，产生并发送读请求（ar-valid）、读地址
// （ar-addr）、读字节数（ar-size）、读个数（ar-len），本次总线支持的宽度是8 Byte，所以如果不是突发
// 传输（burst），一次只能读写8 Byte。当从机（slave）接收到读请求（ar-valid）且处于空闲状态则产生返
// 还给主机（master）读准备(ar-ready)信号表明可以进行读取操作，这时就完成了一次读地址的握手，从机
// （slave）开始准备需要返回的数据（r-data）、读返回请求（r-valid）、读完成（r-last），主机此时也跳变
// 到下一个读数据状态，产生并发出可以读取返回数据的状态读准备（r-ready），当主机的r-valid & r-ready
// 完成握手，主机得到需要的数据（r-data），当主机接收到读完成（r-last）则完成了一次读事务同时状态跳
// 变到空闲状态，并且产生读完成信号（ready），将指令数据（inst）返还给取指模块（IF）。

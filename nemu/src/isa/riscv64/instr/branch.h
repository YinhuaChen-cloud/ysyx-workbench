...
bool isCall(rtlreg_t *dest_reg){
//	//printf("dest_reg = 0x%p\n", dest_reg);
	Assert(dest_reg, "dest_reg == NULL!");
	return dest_reg == (rtlreg_t *)&(cpu.gpr[1]);
}
// if it is jalr, and dsrc1 is x1(ra), then it is ret
bool isRet(rtlreg_t *src1_reg){
	Assert(src1_reg, "src1_reg == NULL!");
	return src1_reg == (rtlreg_t *)&(cpu.gpr[1]);	
}
...
def_EHelper(beq) {
  rtl_jrelop(s, RELOP_EQ, dsrc1, dsrc2, id_dest->simm + s->pc); // if dsrc1 == dsrc2, jump to pc + simm
}

def_EHelper(bne) {
  rtl_jrelop(s, RELOP_NE, dsrc1, dsrc2, id_dest->simm + s->pc); // if dsrc1 == dsrc2, jump to pc + simm
}

def_EHelper(bltu) {
  rtl_jrelop(s, RELOP_LTU, dsrc1, dsrc2, id_dest->simm + s->pc); // if dsrc1 == dsrc2, jump to pc + simm
}

def_EHelper(blt) {
  rtl_jrelop(s, RELOP_LT, dsrc1, dsrc2, id_dest->simm + s->pc); // if dsrc1 == dsrc2, jump to pc + simm
}

def_EHelper(bge) {
  rtl_jrelop(s, RELOP_GE, dsrc1, dsrc2, id_dest->simm + s->pc); // if dsrc1 == dsrc2, jump to pc + simm
}

def_EHelper(bgeu) {
  rtl_jrelop(s, RELOP_GEU, dsrc1, dsrc2, id_dest->simm + s->pc); // if dsrc1 == dsrc2, jump to pc + simm
}

...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, if((src1 + src2) % 4 == 0) R(dest) = (sword_t)(int32_t)Mr(src1 + src2, 4); else s->dnpc = isa_raise_intr(EVENT_UNALIGN_MEM_ACCESS, s->pc););
  INSTPAT("??????? ????? ????? 110 ????? 00000 11", lwu    , I, if((src1 + src2) % 4 == 0) R(dest) = Mr(src1 + src2, 4); else s->dnpc = isa_raise_intr(EVENT_UNALIGN_MEM_ACCESS, s->pc););
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, if((src1 + src2) % 2 == 0) R(dest) = (sword_t)(int16_t)Mr(src1 + src2, 2); else s->dnpc = isa_raise_intr(EVENT_UNALIGN_MEM_ACCESS, s->pc););
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, if((src1 + src2) % 2 == 0) R(dest) = Mr(src1 + src2, 2); else s->dnpc = isa_raise_intr(EVENT_UNALIGN_MEM_ACCESS, s->pc););
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl    , R, R(dest) = src1 >> (src2 & shift_mask));
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00000 11", ld     , I, if((src1 + src2) % 8 == 0) R(dest) = Mr(src1 + src2, 8); else s->dnpc = isa_raise_intr(EVENT_UNALIGN_MEM_ACCESS, s->pc););
